...
module.exports = function (Peer) {
  ...
  *    });
    ...
    Peer.patchFollowing = function (id, data, cb) {
        ...
        Peer.app.models.peer.findById(data.id, function (err, peerInstance) {
            if (err) {
                ...
                console.log("found " + peerInstance.username);
                /*
    ...
    Peer.deleteFollowing = function (id, data, cb) {
        console.log("deleting existing following relation");
        ...
        Peer.app.models.peer.findById(data.id, function (err, peerInstance) {
            if (err) {
                 ...
                 */
                Peer.dataSource.connector.execute(
                    "MATCH (a:peer{id:'" + id + "'})-[r:following]->(b:peer{id:'" + peerInstance.id + "'}) delete r",
                    function (err, results) {
                        if (err) {
                            cb(err);
                        }
    ...
    Peer.patchTopicsRelation = function (id, data, cb) {
        ...
        Peer.app.models.topic.findById(data.id, function (err, topicInstance) {
            if (err) {
                 ...
                 */
                Peer.dataSource.connector.execute(
                    "MATCH (p:peer {id: '" + id + "'}),(t:topic {id: '" + topicInstance.id + "'}) MERGE (p)-[:hasTopic]->(t) RETURN t",
                    function (err, results) {
                        if (err) {
                            cb(err);
                        }
    ...
    Peer.deleteTopicRelation = function (id, data, cb) {

        console.log("deleting existing topic relation");
        ...
        Peer.app.models.topic.findById(data.id, function (err, topicInstance) {
            if (err) {
                 ...
                 */
                Peer.dataSource.connector.execute(
                    "MATCH (a:peer{id:'" + id + "'})-[r:hasTopic]->(t:topic{id:'" + topicInstance.id + "'}) delete r",
                    function (err, results) {
                        if (err) {
                            cb(err);
                        }
    ...
    Peer.patchCollectionsRelation = function (id, data, cb) {
        ...
        Peer.app.models.collection.findById(data.id, function (err, collectionInstance) {
            if (err) {
                 ...
                 */
                Peer.dataSource.connector.execute(
                    "MATCH (p:peer {id: '" + id + "'}),(c:collection {id: '" + collectionInstance.id + "'}) MERGE (p)-[:hasCollection]->(c) RETURN c",
                    function (err, results) {
                        if (err) {
                            cb(err);
                        }
    ...
    Peer.deleteCollectionRelations = function (id, data, cb) {

        console.log("deleting existing collections relations");
        ...
        Peer.app.models.collection.findById(data.id, function (err, collectionInstance) {
            if (err) {
                ...
                console.log("found" + collectionInstance.id);
                /*
                 NEW: Collection -[hasParticipant]-> Peer
                 */
                Peer.dataSource.connector.execute(
                    "MATCH (a:peer{id:'" + id + "'})-[r:hasCollection]->(c:collection{id:'" + collectionInstance.id + "'}) delete r",
                    function (err, results) {
                        if (err) {
                            cb(err);
                        }
    ...
    Peer.setup = function () {
        ...
        //delete the peer - peer following relation
        PeerModel.remoteMethod(
            'deleteFollowing',
            {
                description: 'Delete the peer node relation',
                accepts: [
                    { arg: 'id', type: 'string', http: { source: 'path' } },
                    { arg: 'data', type: 'object', http: { source: 'body' }, required: true }
                ],
                returns: { arg: 'peerObject', type: 'object', root: true },
                http: { verb: 'delete', path: '/:id/following' }
            }
        );
        ...
        PeerModel.remoteMethod(
            'deleteTopicRelation',
            {
                ...
                returns: { arg: 'peerObject', type: 'object', root: true },
                http: { verb: 'delete', path: '/:id/topics' }
            }
        );
        //Remote method for adding topics
        PeerModel.remoteMethod(
            'patchCollectionsRelation',
            {
        ...
        PeerModel.remoteMethod(
            'deleteCollectionRelations',
            {
                ...
                returns: { arg: 'collectionObject', type: 'object', root: true },
                http: { verb: 'delete', path: '/:id/collections' }
            }
        );
