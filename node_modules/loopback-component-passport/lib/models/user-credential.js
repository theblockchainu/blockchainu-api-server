  ...
  UserCredential.link = function(userId, provider, authScheme, profile,
    ...
    userCredentialModel.findOne({where: {
      ...
      if (extCredential) {
        ...
        return extCredential.updateAttributes({profile: JSON.stringify(profile),
          credentials: JSON.stringify(credentials), modified: date}, cb);
      }
      ...
      userCredentialModel.create({
        ...
        console.log("New Credentials saved for user");
        userCredentialModel.dataSource.connector.execute(
            "match (p:peer {id: '" + userId + "'}), (c:userCredential {id: '"+i.id+"'}) merge (p)-[r:hasCredential]->(c) return r",
