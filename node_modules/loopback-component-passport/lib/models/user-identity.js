...
module.exports = function(UserIdentity) {
  ...
  function createAccessToken(user, ttl, cb) {
    ...
    uid(user.constructor.settings.accessTokenIdLength || DEFAULT_TOKEN_LEN, function(err, guid) {
        if (err) {
            cb(err);
        } else {
            user.constructor.dataSource.connector.execute(
                "match (p:peer {username: '" + user.username + "'}) create (p)-[r:hasToken]->(token:UserToken {id: '"+guid+"', ttl: '"+ttl+"', created: timestamp()}) return token",
                cb
            );
        }
    });
  ...
  // Let's create a user for that
    ...
    var email = provider === 'ldap' || profileEmail ? profileEmail : generatedEmail;
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials,
                                 options, cb) {
    console.log("Checking if there is an existing account for this social user... ");
    options = options || {};
    ...
    userIdentityModel.findOne({where: {
      ...
      if (identity) {
        console.log("Existing account found for this social user!");
        identity.credentials = credentials;
        
        return identity.updateAttributes({profile: JSON.stringify(profile),credentials: JSON.stringify(credentials), modified: new Date()}, function(err, i) {
        ...
        userIdentityModel.findOrCreate({where: {externalId: profile.id}}, {
          ...
          if (!err && user && autoLogin) {
            ...
            if(user.isNewInstance)
              ...
              console.log("Found existing account for this social user. Logging in...");
            if(identity && identity.isNewInstance)
              console.log("Created new user-identity entry");

            userModel.dataSource.connector.execute(
                "match (p:peer {username: '" + user.username + "'}), (i:userIdentity {id: '"+identity.id+"'}) merge (p)-[r:hasIdentity]->(i) return r",
