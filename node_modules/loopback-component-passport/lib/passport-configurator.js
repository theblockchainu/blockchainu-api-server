  ...
  if (!noSession) {
    ...
    passport.deserializeUser(function(id, done) {
      ...
      self.userModel.findById(id, function(err, user) {
        ...
        user.identities(user, function(err, identities) {
            console.log("Deserializing user from cookie ID. Identities are: " + identities);
            user.profiles = identities;
            user.credentials(function(err, accounts) {
                console.log("Deserializing user from cookie ID. Accounts are: " + accounts);
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  // Here we define strategy options and verify callback function
  var strategy;
  switch (authType) {
    case 'ldap':
      ...
      options.authOptions), function(err, user, info) {
        ...
        if (session) {
          req.logIn(user, function(err) {
            if (err) {
              console.log("Error trying to log in this user: " + err);
              return next(err);
            }
            if (info && info.accessToken) {
              if (!!options.json) {
                return res.json({
                  'access_token': info.accessToken[0].token.properties.id,
                  userId: user.id,
                });
              } else {
                console.log("received info from delegate server is: " + JSON.stringify(info));
                res.cookie('access_token', info.accessToken[0].token.properties.id,
                  {
                    signed: req.signedCookies ? true : false,
                  // maxAge is in ms
                    maxAge: 1000 * info.accessToken[0].token.properties.ttl,
