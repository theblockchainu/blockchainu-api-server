...
module.exports = function setupCron(server) {
	...
	const tenMinuteCron = new CronJob('00 */10 * * * *',
			...
			function () {
				...
				server.models.message.find({ 'where': { 'and': [{'type': 'user'},{'notifiedUsers': 'false'}] }, 'include': [{ 'peer': 'profiles' }, {'room': [{'participants': 'profiles'}, {'collection': 'owners'}]}, {'readReceipts': 'peer'}] }, function (err, messageInstances) {
					if (!err && messageInstances && messageInstances.length > 0) {
						console.log('Found un-notified messages. Send email to all unread recipients.');
						messageInstances.forEach(messageInstance => {
							// Find all recipients of this message
							const messageParticipants = messageInstance.room()[0].participants();
							...
							const unreadRecipients = _.filter(messageParticipants, (r) => {
								return _.find(messageInstance.readReceipts(), (o) => o.peer[0].id === r.id) === undefined;
							});
							if (unreadRecipients && unreadRecipients.length > 0) {
								unreadRecipients.forEach(unreadRecipient => {
									...
									let message = {
										...
										roomName: messageInstance.room()[0].type === 'group' ? messageInstance.room()[0].name : 'private',
										roomId: messageInstance.room()[0].id
									};
									let renderer = loopback.template(path.resolve(__dirname, '../../server/views/newUnreadMessage.ejs'));
									let html_body = renderer(message);
								...
								messageInstance.updateAttributes({'notifiedUsers': 'true'}, (err, res) => {
									if (err) {
										console.log(err);
									} else {
										console.log('Updated message status to notified');
									}
								});
							} else {
								console.log('No unread recipients for this message');
							}
						});
					} else if (err) {
						console.log(err);
					} else {
						console.log('Found NO un-notified messages.' + messageInstances);
					}
				});
				
				// delete mentor slots booked in last 10 mins but payment not yet done
				const now = moment();
				const query = {
					where: {
						type: 'session'
				...
				server.models.collection.find(query).then((results) => {
					...
					console.log(res.length + ' stale session slots deleted');
				}).catch(err => {
					console.log(err);
				});
				...
				// Update the cached conversion rates of Ether in USD
				request
						.get({
				...
				// Update the cached values of Karma mint rate and Gyan earn rate
				request
						.get({
				...
				// send upcoming mentor session email to student and teacher
